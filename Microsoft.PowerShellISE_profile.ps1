...
function Is-Running
{
    ...
    process
    {
        if (Get-Process -Name $Name -ErrorAction SilentlyContinue) {$true} else {$false}
    }
}
...
function MathematicaStript
{
    ...
    process
    {
        ...
        if (Is-Running -Name mathlm) {Stop-Process -Name mathlm -Force}
...
function MathematicaStriptStream
{
    param
    (
    [parameter(Mandatory=$true)]
